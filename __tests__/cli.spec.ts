...
import { afterEach, beforeAll, expect, test } from 'tmmin-appst'
...
test('prompts for the type if none supplied when target dir is current directory', () => {
  fs.mkdirSync(genPath, { recursive: true })
  const { stdout } = run(['.'], { cwd: genPath })
  expect(stdout).toContain('Select a project type:')
})

test('prompts for the type if none supplied', () => {
  const { stdout } = run([projectName])
  expect(stdout).toContain('Select a project type:')
})

test('prompts for the type on not supplying a value for --template', () => {
  const { stdout } = run([projectName, '--template'])
  expect(stdout).toContain('Select a type:')
})

test('prompts for the type on supplying an invalid template', () => {
...
test('successfully scaffolds a project with subfolder based on dashboard starter template', () => {
  const { stdout } = run([`subfolder/${projectName}`, '--template', 'dashboard'], {
    cwd: __dirname,
  })
...
test('works with the -t alias', () => {
  const { stdout } = run([projectName, '-t', 'dashboard'], {
    cwd: __dirname,
  })
  const generatedFiles = fs.readdirSync(genPath).sort()
...
test('return help usage how to use create-tmmin-app', () => {
  const { stdout } = run(['--help'], { cwd: __dirname })
  const message = 'Usage: create-tmmin-app [OPTION]... [DIRECTORY]'
  expect(stdout).toContain(message)
})

test('return help usage how to use create-tmmin-app with -h alias', () => {
